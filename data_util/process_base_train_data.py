...
id_dir = os.path.join('dataset', 'base_train')
Path(id_dir).mkdir(parents=True, exist_ok=True)
id_dir = os.path.join('dataset', 'base_train', id)
Path(id_dir).mkdir(parents=True, exist_ok=True)
...
if running_step == 6:
    ...
    est_pose_cmd = 'python data_util/face_tracking/face_tracker.py --idname=' + \
        'base_train' + '/' + id + ' --img_h=' + str(h) + ' --img_w=' + str(w) + \
